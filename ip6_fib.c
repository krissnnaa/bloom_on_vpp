...
static int fib_pre_mask = 0;
static int no_of_prefixs = 0;
static f64 time_taken = 0;
static int nroutes = 500000;
...
show_number_of_lookups (vlib_main_t * vm,
                        ...
                        vlib_cli_command_t * cmd)
{
    vlib_cli_output (vm, "Bloom lookups : %d  \n Fib lookups : %d \n Mask : %d \n time taken : %.17g \n No of Masks :%d ",
                     bloom_lookups, fib_lookups, fib_pre_mask, time_taken, no_of_prefixs);

    vlib_cli_output (vm, "No of routes %d"
            , nroutes);
...
create_fib_with_table_id (u32 table_id)
{
    ...
    // Bloom code starts
    create_bloom_filter(&v6_fib->bloom_filter, nroutes);
    // Bloom code ends
                      ...
                      u32 len)
{
    ...
    for (; i < n_p; i++)
    {
        ...
        if(use_bloom) {
            bloom_lookups++;
            if(lookup_of_bloom_filter(v6_fib->bloom_filter, kv.key) == 1) {
                fib_lookups++;
                fib_pre_mask = dst_address_length;
                            ...
                            fib_node_index_t fib_entry_index)
{
    ...
    table->non_empty_dst_address_length_bitmap =
            clib_bitmap_set (table->non_empty_dst_address_length_bitmap,
                             128 - len, 1);
    compute_prefix_lengths_in_search_order (table);
    insert_entries_into_bloom_filter(v6_fib->bloom_filter, kv.key);
}
                             ...
                             const ip6_address_t * dst)
{
    ...
    for (i=0; i < len; i++)
    {
        ...
        if(use_bloom) {
            bloom_lookups++;
            if (lookup_of_bloom_filter(v6_fib->bloom_filter, kv.key) ==
                1) {
                fib_lookups++;
                fib_pre_mask = dst_address_length;
                rv = BV(clib_bihash_search_inline_2)(&table->ip6_hash, &kv, &value);
                                 ...
                                 const dpo_id_t *dpo)
{
    ...
    insert_entries_into_bloom_filte(v6_fib->bloom_filter, kv.key);
}
